...
validate_branch(){
	...
	for repo in ${GH_REPO}; do
		...
		curl --silent -X GET -u ${GH_TOKEN}:x-oauth-basic "https://api.github.com/repos/${GH_ORG}/${repo}/branches" | ../jq/jq --arg branch $NH_BRANCH -c -e '.[] | select(.name|contains($branch))' > /dev/null;
			if [ $? -eq 0 ]; then
			  ...
			  echo -e "INFO: Defaulting to branch develop for repo ${repo}"
			  GH_BRANCH=develop
			  # Call fetch_archive
			  fetch_archive ${repo} ${GH_BRANCH}
			fi;
		echo -e "DONE: Completed actions for branch ${NH_BRANCH} in repo ${repo}";
		echo -e "";
	done
